...
class Trainer_base(BaseTrainer):
    ...
    def _test(self, epoch=None):
        ...
        image_path = str('results_steps'+self.task_info['setting']+'/'+self.task_info['name']+'/'+str(self.task_info['step']))
...
class Trainer_incremental(Trainer_base):
    ...
    def __init__(
        ...
        if config['data_loader']['args']['task']['step'] == 1:
            prev_info_path = \
        ...
        if config['data_loader']['args']['task']['step'] >= 2:
            self.current_numbers = prev_info['current_numbers'].to(self.device)

        else:
            self.current_numbers = self.prev_numbers
        # self.current_numbers = self.numbers[1:].sum().to(self.device)
        ...
        for batch_idx, data in enumerate(self.train_loader):
            ...
            with torch.cuda.amp.autocast(enabled=self.config['use_amp']):
                ...
                if logits_for_extra_bg is not None:
                    extra_bg_label = torch.zeros(1, logits_for_extra_bg.shape[2], 1, requires_grad=False).to(
                        self.device)
                ...
                loss_cont = self.ContLoss(
                    features[-1], logit, data['label'], self.n_new_classes,self.prev_prototypes)
