...
class VOCSegmentationIncremental(BaseDataset):
    ...
    def __init__(
        ...
        _splits_dir = self._base_dir / "splits"
        ...
        if (idxs_path is not None) and (os.path.exists(idxs_path)):
            ...
            # print("idx path", "/".join(idxs_path.split("/")[:-1]))
            os.makedirs("/".join(str(idxs_path).split("/")[:-1]),exist_ok=True) ## shlee added
            ...
            # print("make dir:", "/".join(idxs_path.split("./")[:-1]))
            if (idxs_path is not None) and (distributed.get_rank() == 0):
                if not os.path.exists(idxs_path):
                    # os.makedirs("/".join(idxs_path.split("./")[:-1]))
                    open(idxs_path, "w").close()
    ...
    def __getitem__(self, index):
        ...
        # if sample["image_name"] == '/JPEGImages/2007_000663.jpg /SegmentationClassAug/2007_000663.png':
        #     composed_transforms = tr.Compose(
        #         [
        #             tr.Resize(size=self.transform_args['crop_size']),
        #             tr.CenterCrop(self.transform_args['crop_size']),
        #         ])
        #     img = composed_transforms(sample['image'])
        #     img.save("1.jpg")
        if self.transform:
