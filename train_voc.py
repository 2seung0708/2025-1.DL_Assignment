...
def main_worker(gpu, ngpus_per_node, config):
    ...
    # init_url = f'tcp://{os.environ["MASTER_ADDR"]}:{os.environ["MASTER_PORT"]}'
    init_url = f'tcp://127.0.0.1:{config["port"]}'
    dist.init_process_group(
        backend=config['dist_backend'], init_method=init_url,
        world_size=config['world_size'], rank=config['rank']
    )
    ...
    # print(dataset)
    # import pdb;pdb.set_trace()
    logger.info(f"{dataset.dataset_info()}") ## AttributeError: 'NoneType' object has no attribute 'dataset_info'
    ...
    if task_step > 0:
        if task_step == 1:
            ...
            old_path = config.save_dir.parent / f"step_{task_step - 1}" / f"checkpoint-epoch{config['trainer']['epochs_incre']}.pth"
...
if __name__ == '__main__':
    ...
    options = [
        ...
        CustomArgs(['--port'], type=int, target='port'),
